# Modified version of predict.gcrq() function that requires user to specify 
# the model formula as an argument
# This is used specifically by the k-fold validation function

predict.gcrq.inputform <-
  function(object, newdata, form, se.fit=FALSE, transf=NULL, xreg, type=c("sandw","boot"), ...){
    ...
    if(missing(xreg)){
      if(missing(newdata)) {
        ...
        if(is.null(object$info.smooth)){ #if the model is linear
          ...
          nomiVarModelloRispo<-all.vars(form) #include la risposta che puo' includere piu nome se e scritta come y[g==1]
          nRispo<-length(all.vars(update.formula(form,.~1)))
