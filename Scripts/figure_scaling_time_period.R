...
# Function to create data frame of predictor variables for time period-specific curves
new_data_fn <- function(df){
  t1 <- tibble(log_fire_area = seq(min(df$log_fire_area[df$Time_Period == "Early"]), 
                                      max(df$log_fire_area[df$Time_Period == "Early"]), by = 0.1),
               Time_Period = "Early")
  t2 <- tibble(log_fire_area = seq(min(df$log_fire_area[df$Time_Period == "Late"]), 
                                      max(df$log_fire_area[df$Time_Period == "Late"]), by = 0.1),
               Time_Period = "Late")
  t <- bind_rows(t1, t2) %>%
    mutate(Time_Period = factor(Time_Period))
  return(t)
...
# Function to make predictions for time period-specific curves
predict_time_per_fn <- function(models, df){
...
new_data_dif_fn <- function(df, var){
  # Find overlapping range of fire sizes for both time periods
  size_range <- df %>%
    drop_na(.data[[var]]) %>%
...
predict_time_per_diff_fn <- function(models, df, var){
  # List to store predictions
  dif_list <- list()
  # Dataframe of predictor variables
  ...
  for (i in 1:length(taus)){
    dif_list[[i]] <- smooth_diff(models[[i]], newdata = new_df,
                                 f1 = "Late", f2 = "Early", var = "Time_Period", alpha = 1-conf) %>%
      mutate(tau = taus[i]) %>%
...
for(i in 1:length(taus)){
  mod_high[[i]] <- gcrq(log_patch_area_AW_mean ~ Time_Period + 
                          ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                        lambda0 = 5, tau = taus[i], data = fire_metrics_high)
  mod_mix[[i]] <- gcrq(log_patch_area_AW_mean ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_mix)
  mod_low[[i]] <- gcrq(log_patch_area_AW_mean ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_low)
}

# Make redictions
pred_high <- predict_time_per_fn(mod_high, fire_metrics_high)
pred_mix <- predict_time_per_fn(mod_mix, fire_metrics_mix)
pred_low <- predict_time_per_fn(mod_low, fire_metrics_low)

pred <- bind_rows(pred_high, pred_mix, pred_low) %>%
  mutate(Fire_Regime = factor(Fire_Regime, levels = c("Low", "Mixed", "High"))) 

pred_wide <- pred %>%
  select(tau, fit, log_fire_area, Time_Period, Fire_Regime) %>%
...
p1 = ggplot(pred_wide) +
  ...
  scale_x_continuous(breaks = c(3, 4, 5), labels = c("1,000", "10,000", "100,000"),
                     name = "Fire size (ha)") +
  scale_color_manual(values = c("gray20", "#CC4678FF"), name = "Time period") +
  scale_fill_manual(values = c("gray20", "#CC4678FF"), name = "Time period") +
  geom_point(data =  fire_metrics, 
             aes(x = log_fire_area, y = log_patch_area_AW_mean, color = Time_Period), size = 0.6, alpha = 0.15) +
  geom_ribbon(aes(x = log_fire_area, ymin = `0.05`, ymax = `0.95`, fill = Time_Period), alpha = 0.3) +
  geom_line(aes(x = log_fire_area, y = `0.05`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.95`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.5`, color = Time_Period)) +
  theme_bw() +
...
dif_high <- predict_time_per_diff_fn(mod_high, fire_metrics_high, "log_patch_area_AW_mean")
dif_mix <- predict_time_per_diff_fn(mod_mix, fire_metrics_mix, "log_patch_area_AW_mean")
dif_low <- predict_time_per_diff_fn(mod_low, fire_metrics_low, "log_patch_area_AW_mean")
...
for(i in 1:length(taus)){
  mod_high[[i]] <- gcrq(beta ~ Time_Period + 
                          ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                        lambda0 = 5, tau = taus[i], data = fire_metrics_high)
  mod_mix[[i]] <- gcrq(beta ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_mix)
  mod_low[[i]] <- gcrq(beta ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_low)
}
...
p3 = ggplot(pred_wide) +
  ...
  scale_fill_manual(values = c("gray20", "#CC4678FF"), name = "Time period") +
  geom_point(data =  fire_metrics, 
             aes(x = log_fire_area, y = beta, color = Time_Period), size = 0.6, alpha = 0.15) +
  geom_ribbon(aes(x = log_fire_area, ymin = `0.05`, ymax = `0.95`, fill = Time_Period), alpha = 0.3) +
  geom_line(aes(x = log_fire_area, y = `0.05`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.95`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.5`, color = Time_Period)) +
...
# Predictions for differences between curves
dif_high <- predict_time_per_diff_fn(mod_high, fire_metrics_high, "beta")
dif_mix <- predict_time_per_diff_fn(mod_mix, fire_metrics_mix, "beta")
dif_low <- predict_time_per_diff_fn(mod_low, fire_metrics_low, "beta")
...
for(i in 1:length(taus)){
  mod_high[[i]] <- gcrq(psi ~ Time_Period + 
                          ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                        lambda0 = 5, tau = taus[i], data = fire_metrics_high)
  mod_mix[[i]] <- gcrq(psi ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_mix)
  mod_low[[i]] <- gcrq(psi ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_low)
}
...
p5 = ggplot(pred_wide) +
  ...
  scale_fill_manual(values = c("gray20", "#CC4678FF"), name = "Time period") +
  geom_point(data =  fire_metrics, 
             aes(x = log_fire_area, y = psi, color = Time_Period), size = 0.6, alpha = 0.15) +
  geom_ribbon(aes(x = log_fire_area, ymin = `0.05`, ymax = `0.95`, fill = Time_Period), alpha = 0.3) +
  geom_line(aes(x = log_fire_area, y = `0.05`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.95`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.5`, color = Time_Period)) +
...
# Predictions for differences between curves
dif_high <- predict_time_per_diff_fn(mod_high, fire_metrics_high, "psi")
dif_mix <- predict_time_per_diff_fn(mod_mix, fire_metrics_mix, "psi")
dif_low <- predict_time_per_diff_fn(mod_low, fire_metrics_low, "psi")
...
for(i in 1:length(taus)){
  mod_high[[i]] <- gcrq(log_total_core ~ Time_Period + 
                          ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                        lambda0 = 5, tau = taus[i], data = fire_metrics_high)
  mod_mix[[i]] <- gcrq(log_total_core ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_mix)
  mod_low[[i]] <- gcrq(log_total_core ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = 1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_low)
}
...
p7 = ggplot(pred_wide) +
  ...
  scale_fill_manual(values = c("gray20", "#CC4678FF"), name = "Time period") +
  geom_point(data =  fire_metrics, 
             aes(x = log_fire_area, y = log_total_core, color = Time_Period), size = 0.6, alpha = 0.15) +
  geom_ribbon(aes(x = log_fire_area, ymin = `0.05`, ymax = `0.95`, fill = Time_Period), alpha = 0.3) +
  geom_line(aes(x = log_fire_area, y = `0.05`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.95`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.5`, color = Time_Period)) +
...
# Predictions for differences between curves
dif_high <- predict_time_per_diff_fn(mod_high, fire_metrics_high, "log_total_core")
dif_mix <- predict_time_per_diff_fn(mod_mix, fire_metrics_mix, "log_total_core")
dif_low <- predict_time_per_diff_fn(mod_low, fire_metrics_low, "log_total_core")
...
for(i in 1:length(taus)){
  mod_high[[i]] <- gcrq(log_SDC ~ Time_Period + 
                          ps(log_fire_area, shared.pen = TRUE, monotone = -1, by = Time_Period),
                        lambda0 = 5, tau = taus[i], data = fire_metrics_high)
  mod_mix[[i]] <- gcrq(log_SDC ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = -1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_mix)
  mod_low[[i]] <- gcrq(log_SDC ~ Time_Period +
                         ps(log_fire_area, shared.pen = TRUE, monotone = -1, by = Time_Period),
                       lambda0 = 5, tau = taus[i], data = fire_metrics_low)
}
...
p9 = ggplot(pred_wide) +
  ...
  scale_fill_manual(values = c("gray20", "#CC4678FF"), name = "Time period",
                    labels = c("Early\n(1985 – 2000)", "Late\n(2001 – 2020)")) +
  geom_point(data =  fire_metrics, 
             aes(x = log_fire_area, y = log_SDC, color = Time_Period), size = 0.6, alpha = 0.15) +
  geom_ribbon(aes(x = log_fire_area, ymin = `0.05`, ymax = `0.95`, fill = Time_Period), alpha = 0.3) +
  geom_line(aes(x = log_fire_area, y = `0.05`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.95`, color = Time_Period), lwd = 0.1) +
  geom_line(aes(x = log_fire_area, y = `0.5`, color = Time_Period)) +
...
# Predictions for differences between curves
dif_high <- predict_time_per_diff_fn(mod_high, fire_metrics_high, "log_SDC")
dif_mix <- predict_time_per_diff_fn(mod_mix, fire_metrics_mix, "log_SDC")
dif_low <- predict_time_per_diff_fn(mod_low, fire_metrics_low, "log_SDC")
...
# Export figure
ggsave(paste0("Figures/figureS3_scaling_time_period.png"),
